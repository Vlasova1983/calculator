...
const buttons:string=arrayButtons.map(item=>`<button class="btn" id="${item.id}">${item.content}</button>`).join(''); // розмітка кнопок
const solutionBlock:string=`<p class="solution" id ="screen">0<p/>`// розмітка екрана результата рахувань
const calculatorScreen:string = `<div class="calculator_conteiner">
